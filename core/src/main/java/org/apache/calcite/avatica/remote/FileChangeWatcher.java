 ...
 * "https://github.com/apache/zookeeper/blob/8148f966947d3ecf3db0b756d93c9ffa88174af9
 ...
 * When a file is added to, deleted from, or is modified in the given directory,
 * the callback provided by the user will be called from a background thread.
...
public final class FileChangeWatcher {
  ...
  /**
   * Creates a watcher that watches <code>dirPath</code>
   * and invokes <code>callback</code> on changes.
   *
   * @param dirPath the directory to watch.
   * @param callback the callback to invoke with events.
   *                 <code>event.kind()</code> will return the type of event, and
   *                 <code>event.context()</code> will return the filename relative to
   *                 <code>dirPath</code>.
  ...
  public FileChangeWatcher(Path dirPath, Consumer<WatchEvent<?>> callback) throws IOException {
    ...
    dirPath.register(watchService, new WatchEvent.Kind<?>[] { StandardWatchEventKinds.ENTRY_CREATE,
  ...
  private class WatcherThread extends Thread {
    ...
    @Override public void run() {
      try {
        ...
        if (!compareAndSetState(FileChangeWatcher.State.STARTING,
    ...
    private void runLoop() {
      while (FileChangeWatcher.this.getState() == FileChangeWatcher.State.RUNNING) {
        ...
        for (WatchEvent<?> event : key.pollEvents()) {
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                "Got file changed event: " + event.kind() + " with context: " + event.context());
          }
          try {
        ...
        if (!isKeyValid) {
          ...
          // directory we are watching was deleted or otherwise became inaccessible
          // (unmounted, permissions changed, ???).
